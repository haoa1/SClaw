...
# -*- coding: utf-8 -*-
"""
backtest_limitup_pullback_v2.py — 涨停缩量回调策略 · 调优后正式版
============================================================
最终参数 (2026-08-22 调优确定, 全周期回测 +108.57% / 年化18.8% / 回撤28.3% / pf1.46):
...
用法:
  python3 backtest_limitup_pullback_v2.py          # 全量回测 + 分段统计
  python3 backtest_limitup_pullback_v2.py signal   # 只输出今日信号股
...
def load_data():
    ...
    return info, cal, date_to_idx, stock_bars, bma, is_lu


def build_signals(info, cal, stock_bars, bma, is_lu):
...
def main():
    info, cal, _, stock_bars, bma, is_lu = load_data()
    print("Building signals...", flush=True)
    sigs = build_signals(info, cal, stock_bars, bma, is_lu)
