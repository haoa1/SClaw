...
// ====== Strategy 1: 底分型选股 ======
function executeFractalStrategy(data, _params) {
    const results = [];
...
// ====== Strategy 2: 底背驰选股 ======
function executeDivergenceStrategy(data, _params) {
    const results = [];
    for (const item of data) {
        const kline = item.kline;
...
// ====== Strategy 3: 第三类买点 ======
function executeThirdBuyStrategy(data, params) {
    const results = [];
    // 参数：中枢周期 (日线默认20)
    const pivotPeriod = params.pivotPeriod || 20;
    for (const item of data) {
        const kline = item.kline;
        if (!kline || kline.length < pivotPeriod * 2)
            continue;
