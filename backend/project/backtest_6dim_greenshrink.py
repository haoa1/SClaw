#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""backtest_6dim_greenshrink.py — 六维复合选股回测 (低内存事件版)
用户需求: 用6个配合指标制作选股策略, 选出来就买, 回测第二天开盘价卖, 本金10000, 买前三
核心: 日线MACD绿柱从峰值连续向0轴靠近(未翻红, 贴0轴)
六维: [1]绿柱临界 [2]缩量 [3]站上MA5 [4]结构(不处60日高位) [5]大盘相对强度 [6]RSI未超买
买卖: 信号日收盘买, 次日开盘卖
低内存: 逐股流式计算, 只保留事件
"""
import sqlite3
...
def pass_breadth(conn, codes, cal, d2i):
    """compute market 5d momentum (median return) streaming"""
    t0 = time.time()
    n = len(cal)
...
def pass_signals(conn, codes, cal, d2i, mkt5):
    t0 = time.time()
    n = len(cal)
    events = []  # (buy_day_idx, score, code, buy_px, sell_px, sell_day_idx)
    for c in codes:
        rows = stock_rows(conn, c)
        if len(rows) < 30:
            continue
        ...
        for j in range(30, m):
            ...
            if np.isnan(high60[j]) or close[j] > POSITION_HIGH60 * high60[j]:
                continue
            if np.isnan(rsi[j]) or rsi[j] >= RSI_MAX:
                continue
            dj = di_arr[j]
            ...
            score += max(0.0, (MAX_ABS_HIST - abs(hist[j])) / MAX_ABS_HIST) * 20
            vol_r = vol[j] / avg_vol
            ...
            if j > 0 and close[j] > close[j - 1]:
                score += 5
            if not np.isnan(rsi[j]) and 35 <= rsi[j] <= 55:
                score += 5
            if 2 <= turn[j] <= 12:
                score += 5
            # sell at next available bar's open
            if j + 1 >= m:
                continue
            sell_px = open_[j + 1]
            sell_di = di_arr[j + 1]
...
def backtest(events, cal):
    ...
    cash = INIT_CAP
    pos = []  # (code, qty, buy_px, sell_px, sell_di)
    trades = []
    # event-based equity: record after each trading day
    eq_records = []  # (day_idx, equity)
    eq = INIT_CAP
...
def main():
    print("=== 六维复合选股回测 (绿柱临界+缩量+MA5+结构+逆势+RSI) ===", flush=True)
    ...
    with open('/root/sclaw/data/backtest_6dim_result.json', 'w') as f:
        json.dump({'metrics': m, 'segments': seg}, f, ensure_ascii=False, indent=2)
    print("\nsaved -> data/backtest_6dim_result.json")
