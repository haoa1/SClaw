...
Use tools when you need to perform actions. Be concise and helpful.`,
  ...
  private rebuildSystemMessage(): void {
    // ⚠️ SOUL.md 读取必须单独 try：原先整段包在 try 里，SOUL.md 缺失时异常直接跳到 catch，
    // 连带把「技能注入 + 写回 messages[0]」一起吞掉 → 技能静默丢失（测试 expected 'Base prompt.'）
    let soulContent = '';
    try {
      soulContent = fs.readFileSync(this.config.soulPath, 'utf-8').trim();
    } catch {
