# -*- coding: utf-8 -*-
"""
爆款回调·绿柱收敛 双策略回测 v4 — 核心修复: 卖出去抖
  改进1: 大盘过滤 close>MA200 (年线, 模式D)
  改进2: 硬止损 -5%
  改进3: 翻红确认入场
  改进4(新): 卖出去抖 FADE_CONFIRM=2 — hist 需连续2根递减才触发 fade 卖出
             (v3 发现: 1根回落就卖导致大量1-2天被洗出, 4+天持仓胜率86%)
"""
...
CONFIRM_MAX_DAYS = 10
FADE_CONFIRM = 2           # 改进4: 连续N根hist递减才卖
...
if USE_MA200_FILTER:
    ...
    idx_allow = {d: True for d in dates_i}
n_allow = sum(1 for v in idx_allow.values() if v)
print(f'index: {len(dates_i)} days, 年线过滤启用 可开仓={n_allow}天 ({n_allow/len(dates_i)*100:.0f}%)', flush=True)
...
def run_backtest(strategy_id):
    ...
    for gidx, date in enumerate(all_dates):
        for pos in list(positions):
            ...
            # 改进4: fade 卖出需连续 FADE_CONFIRM 根 hist 递减
            if i >= FADE_CONFIRM:
                fade = all(meta['hist'][i - FADE_CONFIRM + 1 + j] < meta['hist'][i - FADE_CONFIRM + j] for j in range(FADE_CONFIRM))
                if fade:
                    hi_prev = meta['hist'][i - FADE_CONFIRM + 1]
                    if strategy_id == 1:
                        if hi_prev >= 0 or True:
                            sell_reason, raw_price = 'green/red-fade', float(meta['c'][i])
                    else:
                        if hi_prev >= 0:
                            sell_reason, raw_price = 'red-fade', float(meta['c'][i])
