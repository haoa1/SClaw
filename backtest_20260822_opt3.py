# -*- coding: utf-8 -*-
"""
爆款回调·绿柱收敛 双策略回测 v3 — 多模式大盘过滤对比
  模式A: 不过滤
  模式B: DIF>DEA(红柱)
  模式C: DIF>0且DEA>0
  模式D: close>MA200 (年线)
一次扫描数据, 四种过滤各跑一遍, 输出对比表
"""
...
di, de, hi = macd_hist(closes_i)
ma200 = np.full_like(closes_i, np.nan)
if len(closes_i) >= 200:
    s = np.convolve(closes_i, np.ones(200)/200, mode='valid')
    ma200[199:] = s
idx_allow = {
    'A': {d: True for d in dates_i},
    'B': {d: (h > 0) for d, h in zip(dates_i, hi)},
    'C': {d: (dif > 0 and dea > 0) for d, dif, dea in zip(dates_i, di, de)},
    'D': {d: (not np.isnan(m) and c > m) for d, c, m in zip(dates_i, closes_i, ma200)},
}
print(f'index: {len(dates_i)} days, 模式A/B/C/D 就绪', flush=True)
...
def run_backtest(strategy_id, allow):
    ...
    for gidx, date in enumerate(all_dates):
        ...
        if len(positions) < MAX_POS and date in signals and allow.get(date, False):
    ...
    return dict(final=round(final,2), total_ret=round(total_ret,2), annual=round(ann,2),
        max_dd=round(mdd,2), trades=len(trades), win_rate=round(win_rate,1),
        reasons=dict(reasons), trades_list=trades)

mode_names = {'A': '不过滤', 'B': 'DIF>DEA红柱', 'C': 'DIF>0且DEA>0', 'D': 'close>MA200'}
results = {}
print('\n' + '='*90)
print(f"{'过滤模式':<16}{'策略':<5}{'总收益%':<10}{'年化%':<9}{'最大回撤%':<11}{'交易数':<7}{'胜率%':<8}{'hard-stop':<10}")
print('-'*90)
for mode in ['A','B','C','D']:
    for sid in [1,2]:
        r = run_backtest(sid, idx_allow[mode])
        results[f'{mode}{sid}'] = r
        hs = r['reasons'].get('hard-stop', 0)
        print(f"{mode_names[mode]:<16}{'S'+str(sid):<5}{r['total_ret']:<10.2f}{r['annual']:<9.2f}{r['max_dd']:<11.2f}{r['trades']:<7}{r['win_rate']:<8}{hs:<10}")
    print('-'*90)

with open('/root/sclaw/data/backtest_20260822_result_v3.json','w') as f:
    json.dump(results, f, ensure_ascii=False, indent=1)
print(f'saved v3. total {time.time()-t0:.0f}s, peak RSS={rss_mb():.0f}MB')
