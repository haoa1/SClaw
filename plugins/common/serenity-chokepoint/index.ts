  ...
  strategies: [
    {
      ...
      execute(data: StockData[], params: Record<string, any>): FilterResult[] {
        ...
        const minChg = (params.minChangePercent as number) ?? 0;
        const minVolRatio = (params.minVolumeRatio as number) ?? 0.5;

        for (const item of data) {
          // Layer 1: 小市值 — Serenity 强调 < $3B (约 210亿RMB以内)
          const mcap = item.marketCap ?? 0;
          const mcapYi = mcap / 100000000;
          if (mcapYi > maxMcapYi || mcapYi <= 0) continue;
          ...
          // 市值越小越好（被忽视阶段加分）
          if (mcapYi < 30) score += 20;         // < $500M — 秘密发现期
          else if (mcapYi < 100) score += 10;   // < $1.5B — 早期发现
          else score += 5;
