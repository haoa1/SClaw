  ...
  strategies: [
    {
      ...
      execute(data: StockData[], params: Record<string, any>): FilterResult[] {
        ...
        for (const s of data) {
          ...
          if (to < minTo) continue;
          if (price > maxPr || price <= 2) continue;
          ...
          // 偏离度不够的过滤掉
          if (absDeviation < minDev) continue;
          ...
          let devScore = 0;
          if (absDeviation >= 4) devScore = 35;
          else if (absDeviation >= 3) devScore = 30;
          else if (absDeviation >= 2) devScore = 25;
          else if (absDeviation >= 1.5) devScore = 20;
          else devScore = 15;
          score += devScore;
          ...
          if (amp >= 6) { score += 25; signals.push('振幅极大'); }
          else if (amp >= 4) { score += 20; signals.push('振幅较大'); }
          else if (amp >= 3) { score += 15; signals.push('振幅适中'); }
          metrics.amplitude = parseFloat(amp.toFixed(2));
          
          // 3. 换手评分 (流动性好容易成交) - 最高20分
          if (to >= 8) { score += 20; signals.push('换手极高'); }
          else if (to >= 5) { score += 18; signals.push('换手活跃'); }
          else if (to >= 3) { score += 15; signals.push('换手充足'); }
          else if (to >= 2) { score += 10; signals.push('换手一般'); }
          metrics.turnover = to;
          
          // 4. 价格评分 (中等价格最适做T) - 最高10分
          if (price >= 10 && price <= 30) { score += 10; signals.push('价格适中'); }
          else if (price > 5 && price < 10) { score += 7; signals.push('低价股'); }
          else if (price > 30 && price <= 50) { score += 5; signals.push('价格偏高'); }
          metrics.price = price;
          
          // 5. 涨幅加分 (非涨停最安全) - 最高10分
          const chg = s.changePercent || s.changePct || 0;
          if (chg > -5 && chg < 7) { score += 10; signals.push('波动空间充足'); }
          else { score += 3; }
          metrics.changePercent = chg;
          
          // 生成日内做T建议
          let tAdvice = '';
          if (deviation > 2) tAdvice = '⚡ 建议：先高抛，等回落后再接回';
          else if (deviation < -2) tAdvice = '⚡ 建议：先低吸，等拉高后卖出';
          else tAdvice = '⚡ 建议：观察等待更好时机';
