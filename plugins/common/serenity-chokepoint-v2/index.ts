  ...
  strategies: [
    {
      ...
      execute(data: StockData[], params: Record<string, any>): FilterResult[] {
        ...
        const includeBJ = (params.includeBJ as boolean) ?? false;
        const excludeST = (params.excludeST as boolean) ?? true;
        const minMcapYi = (params.minMarketCapYi as number) ?? 10;
        const maxMcapYi = (params.maxMarketCapYi as number) ?? 100;
        const minPrice = (params.minPrice as number) ?? 5;
        const minChg = (params.minChangePercent as number) ?? 1;
        const minVolRatio = (params.minVolumeRatio as number) ?? 0.8;

        for (const item of data) {
          // ── Layer 1: 市场排除 ──
          if (!includeBJ && item.market === 'BJ') continue;
          if (excludeST && item.name && (item.name.startsWith('ST') || item.name.startsWith('*ST'))) continue;

          // ── Layer 6: 流动性 + 市值 ──
          const mcap = item.marketCap ?? 0;
          const mcapYi = mcap / 100000000;
          if (mcapYi > maxMcapYi || mcapYi < minMcapYi) continue;
          ...
          if (volRatio > 0) signals.push('量比' + volRatio.toFixed(2));
          const tr = item.turnoverRate ?? 0;
          if (tr > 0 && tr < 50) signals.push('换手' + tr.toFixed(1) + '%');
          if (item.limitUpIn20Days) signals.push('20日涨停🦀');
          if (item.priceAboveVwap) signals.push('均价线上');

          // ── 评分 ──
          let score = 50;
          // 小市值加分 (越快接近上限越扣分)
          if (mcapYi < 30) score += 20;
          else if (mcapYi < 60) score += 15;
          else if (mcapYi < 100) score += 10;
          else score += 5;
          ...
          else if (chg > 2) score += 5;

          // 换手率 (非妖股范围)
          if (tr > 3 && tr < 15) score += 5;
          else if (tr >= 15) score -= 5; // 换手过高 → 游资炒作嫌疑
