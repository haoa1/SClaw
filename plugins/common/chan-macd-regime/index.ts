...
// --- 缠论底背离 + MACD 打分 ---
function scoreStock(st: StockData, params: Record<string, any>): { score: number; signals: string[]; chanOk: boolean } {
  const kl = st.kline;
  if (!kl || kl.length < 55) return { score: 0, signals: [], chanOk: false };
  ...
  return { score: Math.max(0, score), signals, chanOk: priceNewLow && difDivergence };
}
  ...
  strategies: [
    {
      id: 'chan-macd-regime',
      name: '缠论MACD·市场择时',
      description: '市场在 regime_ma30+trend90 多头区才出手 + 个股必须存在缠论底背离（硬门槛，requireChanDefl）',
      category: 'reversal',
      params: [
        ...
        { key: 'requireChanDefl', name: '缠论底背离硬门槛', type: 'boolean', default: true },
      ...
      execute(data: StockData[], params: Record<string, any>): FilterResult[] {
        ...
        const requireChanDefl = params.requireChanDefl !== false; // 硬门槛：必须存在缠论底背离
        ...
        for (const s of data) {
          ...
          const { score, signals, chanOk } = scoreStock(s, params);
          if (requireChanDefl && !chanOk) continue; // 无缠论底背离 → 直接淘汰（不再靠 MACD/MA20 加分项凑分）
