...
/** 中枢分析（第17课）：寻找最近的中枢区间 */
function analyzeZhongshu(kline: KLineData[]): { exists: boolean; upper: number; lower: number; clarity: number } {
  if (!kline || kline.length < 20) return { exists: false, upper: 0, lower: 0, clarity: 0 };
...
function analyzeBeichi(kline: KLineData[]): { hasBeichi: boolean; type: string; detail: string } {
  if (!kline || kline.length < 30) return { hasBeichi: false, type: '数据不足', detail: '需至少30根K线' };
...
/** 三类买点分析（第25课） */
function analyzeBuyPoints(kline: KLineData[], zhongshu: { exists: boolean; upper: number; lower: number }): { level: string; price: number; description: string } {
  if (!kline || kline.length < 20) return { level: '数据不足', price: 0, description: '' };
...
/** 三类卖点分析（第29课） */
function analyzeSellPoints(kline: KLineData[], zhongshu: { exists: boolean; upper: number; lower: number }): { level: string; price: number; description: string; warning: string } {
  if (!kline || kline.length < 20) return { level: '数据不足', price: 0, description: '', warning: '' };
...
/** 缠论综合评分 */
function computeChanScore(
  kline: KLineData[],
  zhongshu: { exists: boolean; clarity: number },
  beichi: { hasBeichi: boolean; type: string },
  buyPoint: { level: string },
  hasLimitUp: boolean
): { total: number; breakdown: { zhongshu: number; beichi: number; buyPoint: number; limitUp: number } } {
  let zhongshuScore = 0;
  let beichiScore = 0;
  let buyPointScore = 0;
  let limitUpScore = 0;
  ...
  // 买点评分（满分30）
  if (buyPoint.level.includes('第三类买点')) buyPointScore = 30;
  else if (buyPoint.level.includes('第二类买点')) buyPointScore = 22;
  else if (buyPoint.level.includes('第一类买点')) buyPointScore = 25;
  else if (buyPoint.level.includes('突破中枢')) buyPointScore = 18;
  else buyPointScore = 10;
  ...
  if (hasLimitUp) {
    ...
    if (kline && kline.length >= 10) {
      ...
      for (let i = kline.length - 1; i >= Math.max(0, kline.length - 10); i--) {
        ...
        if (chg >= 9.5) consecutiveLimitUp++;
        else break;
      }
      if (consecutiveLimitUp >= 2) limitUpScore = 20;
    }
  }
...
function executeChan5FilterStrategy(data: StockData[], params: Record<string, any>): FilterResult[] {
  ...
  for (const item of data) {
    ...
    if (!verifiedLimitUp && item.kline && item.kline.length >= 5) {
      ...
      for (const k of recentK) {
        if ((k.changePct || 0) >= 9.5) { verifiedLimitUp = true; break; }
      }
    }
    // 不强制要求涨停基因，但会影响评分
    ...
    if (verifiedLimitUp) signals.push('涨停基因✓');
    signals.push(`量比${volRatio.toFixed(2)}`);
    signals.push(`换手${tr.toFixed(1)}%`);
    signals.push(`市值${mcapYi.toFixed(0)}亿`);
    if (zhongshu.exists) signals.push(`中枢${zhongshu.lower.toFixed(1)}~${zhongshu.upper.toFixed(1)}`);
    if (beichi.hasBeichi) signals.push(beichi.type);
    signals.push(buyPoint.level);
    if (!sellPoint.level.includes('暂无')) signals.push(sellPoint.warning);
